...
class OpenAiService
{
    ...
    /**
     * Generate a ToS for a named term (wrapper)
     *
     * This helper simply calls generateToS and returns the parsed array with an extra key: 'term'.
     * You may choose to use this if you'd like the ToS to be generated by the LLM instead of the job-local algorithm.
     *
     * @param array $learningOutcomes
     * @param string $materialSummary
     * @param int $totalItems
     * @param string $term 'mid_term'|'final_term'
     * @return array
     */
    public function generateTermToS(array $learningOutcomes, string $materialSummary, int $totalItems = 20, string $term = 'mid_term'): array
    {
        $tos = $this->generateToS($learningOutcomes, $materialSummary, $totalItems);
        if (!is_array($tos)) {
            $tos = [];
        }
        $tos['term'] = $term;
