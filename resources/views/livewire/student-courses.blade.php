    ...
    <div class="rounded-2xl border border-slate-200/70 bg-white/80 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
        <nav class="flex gap-2 rounded-t-2xl bg-slate-100/70 px-4 py-2 dark:bg-slate-800/70">
            <button
                ...
                class="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900 {{ $activeTab === 'enrolled' ? 'border border-indigo-400/50 bg-indigo-500/20 text-indigo-600 dark:border-indigo-400/50 dark:bg-indigo-500/20 dark:text-indigo-200' : 'border border-transparent text-slate-500 hover:border-slate-300 hover:bg-white hover:text-slate-700 dark:text-slate-400 dark:hover:border-slate-700 dark:hover:bg-slate-800/60 dark:hover:text-slate-200' }}"
            >
                My Enrolled Courses ({{ $enrolledCourses->count() }})
            </button>
            <button
                wire:click="$set('activeTab', 'available')"
                class="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900 {{ $activeTab === 'available' ? 'border border-indigo-400/50 bg-indigo-500/20 text-indigo-600 dark:border-indigo-400/50 dark:bg-indigo-500/20 dark:text-indigo-200' : 'border border-transparent text-slate-500 hover:border-slate-300 hover:bg-white hover:text-slate-700 dark:text-slate-400 dark:hover:border-slate-700 dark:hover:bg-slate-800/60 dark:hover:text-slate-200' }}"
            >
            ...
            @if($activeTab === 'enrolled')
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    @forelse($enrolledCourses as $course)
                        <div class="group flex h-full flex-col rounded-2xl border border-slate-200/70 bg-white/90 p-6 shadow-sm transition hover:-translate-y-1 hover:border-indigo-200 hover:shadow-xl dark:border-slate-800/70 dark:bg-slate-900/70 dark:hover:border-indigo-500/40">
                            ...
                            <div class="mb-4 space-y-2 text-sm text-slate-600 dark:text-slate-300">
                                ...
                                <p class="flex items-center gap-2 font-medium">
                                    <svg class="h-4 w-4 text-indigo-500 dark:text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="mt-auto space-y-3">
                                <a
                                    ...
                                    class="block w-full rounded-xl bg-indigo-600 px-4 py-3 text-center text-sm font-semibold text-white shadow transition hover:bg-indigo-500 dark:bg-indigo-500 dark:hover:bg-indigo-400"
                                >
                                    View Course
                                </a>
                                <button
            ...
            @if($activeTab === 'available')
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    @forelse($availableCourses as $course)
                        <div class="group flex h-full flex-col rounded-2xl border border-indigo-200/50 bg-white/90 p-6 shadow-sm transition hover:-translate-y-1 hover:border-indigo-300 hover:shadow-xl dark:border-indigo-500/40 dark:bg-slate-900/70 dark:hover:border-indigo-400">
                            <div class="mb-4 space-y-2">
                                <h3 class="text-xl font-semibold text-slate-900 dark:text-slate-100">{{ $course->course_code }}</h3>
                                <p class="text-sm font-medium text-slate-600 dark:text-slate-300">{{ $course->course_title }}</p>
                                @if($course->description)
                            ...
                            <div class="mb-4 space-y-2 text-sm text-slate-600 dark:text-slate-300">
                                ...
                                @if($course->obtlDocument)
                                    <span class="inline-flex items-center gap-1 rounded-full bg-indigo-500/20 px-3 py-1 text-xs font-semibold text-indigo-600 dark:bg-indigo-500/25 dark:text-indigo-200">
                                        ✓ OBTL Available
                                    </span>
                                @endif
                            </div>
                            ...
                            <button
                                ...
                                class="mt-auto w-full rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 px-4 py-3 text-center text-sm font-semibold text-white shadow transition hover:from-indigo-500 hover:to-blue-500 dark:from-indigo-500 dark:to-blue-500 dark:hover:from-indigo-400 dark:hover:to-blue-400"
                            >
                                🎓 Enroll in Course
                            </button>
                        </div>
